...
|---|---|
| **LangChain** — "built a production-shaped RAG pipeline with LangChain" | LCEL chain, custom `BaseRetriever`, `with_structured_output`, text splitters, Chroma integration — in a public repo with tests |
...
- Multi-agent orchestration (LangGraph/CrewAI), Bedrock, any UI, hybrid
  search, re-ranking, fine-tuning, PDF parsing in the eval path, real
  carrier data, anything from the Freightify codebase.
...
[QueryPlanner]  deterministic: detect carrier alias → metadata filter; no LLM
   │
   ▼
[RateRetriever (BaseRetriever)] ── StoreBackend: ChromaBackend │ PineconeBackend ──► top-k chunks + scores
   │
   ▼
...
|---|---|---|
| `ingest/` | loaders, structure-aware chunking, chunk ids, metadata | `langchain_core.documents`, `langchain_text_splitters` |
...
| `guardrails/` | Gate 1, Gate 2, Gate 3, confidence, the `run_gates()` pipeline | `schema/`, `config/` loaders, stdlib only |
...
asserts byte-equality with the committed files (the corpus is frozen).
...
### 6.3 File 1 — `meridian_tariff_2026_h2.md` (current, Markdown)
...
Numbers in Markdown use a thousands separator (`1,240`); the CSV does
not. Both variants must be recognised by Gate 3.
...
### 6.6 File 4 — `meridian_tariff_2026_q2.md` (superseded trap)

Identical structure to file 1 with: reference `MER-2026-Q2-FCL`, valid
...
```json
{
  ...
  "files": {"meridian_tariff_2026_h2.md": "<sha256>", "...": "..."},
  "rate_values": [1240, 2180, "... all 150 ..."],
  "lane_ranges": {"MERIDIAN|INMAA|NLRTM|40HC": {"min": 2310, "max": 2310, "docs": ["meridian_tariff_2026_h2.md", "meridian_tariff_2026_q2.md"]}}
}
```
...
- id: G-001
  ...
  expected:
    ...
    source_doc: meridian_tariff_2026_h2.md
```
...
|---|---|---|
| `tariff_md` | header block (all bullet lines) + table header row + **4 lane rows** per chunk; last chunk may have fewer; `## Remarks` becomes its own chunk with the header block prepended | 5 rate chunks + 1 remarks chunk per tariff file |
...
also contains `valid_to` (via the replicated header) — this is what makes
...
`RateRetriever(BaseRetriever)` wraps a backend + the embedding wrapper and
implements `_get_relevant_documents(query, *, run_manager)`; it attaches
`similarity_norm` and `rank` into each returned `Document.metadata` so the
chain and the gates never touch the backend directly.
...
score = w_model · candidate.confidence
      + w_sim   · similarity_norm(source_chunk)
      + w_rank  · (1 / rank(source_chunk))          # rank 1 → 1.0, rank 2 → 0.5 …
weights default: w_model 0.4, w_sim 0.4, w_rank 0.2   (config/guardrails.yaml)
```
...
rate-rag ask    "question" --store chroma [--as-of 2026-09-01] [--no-gates] [--json]
rate-rag eval   --store chroma|pinecone|both --mode gated|baseline|both [--set golden|adversarial|all] [--no-cache] [--tune-threshold]
```

`eval --store both --mode both --set all` is the one command that
produces the README table.

### 13.2 Result files (D-20)

`eval/results/<YYYYMMDDTHHMMSSZ>-<store>-<mode>.json`:

```json
{
  "run_id": "20261003T093000Z-chroma-gated",
  "config": {"chat_model": "...", "embedding_model": "...", "dimension": 768, "prompt_version": "1",
             "corpus_version": 1, "as_of": "2026-09-01", "threshold": 0.71, "gates": ["schema","rules","grounding","confidence"]},
  "git_sha": "...",
  "metrics": {...},
...
| `retrieval_recall@k` | `#{q ∈ G_a : expected source doc's owning chunk ∈ top-k} / |G_a|` (LLM-free; diagnoses retrieval vs. gating failures) | reported |
...
- `retrieval.yaml` — `k: 6`, chunk rows per chunk (4 / 6).

### 14.3 `.gitignore` additions

`.env`, `.env.*`, `!.env.example`, `*.key`, `.venv/`, `__pycache__/`,
`*.pyc`, `.chroma/`, `.cache/`, `.pytest_cache/`, `.ruff_cache/`,
...
├── config/                       ← §14.2
├── data/
│   ├── corpus/                   ← 4 files + manifest.json
│   └── eval/                     ← golden.yaml, adversarial.yaml
├── scripts/
│   └── generate_corpus.py        ← seeded generator (also `rate-rag corpus`)
├── src/logistics_rate_rag/
│   ├── __init__.py
│   ├── config.py                 ← env + YAML loading, one Settings object
│   ├── ingest/   loaders.py, chunking.py
...
| `test_gate3_confidence.py` | weight arithmetic; rank 1/2/3; threshold boundary (equal passes); `NEEDS_REVIEW` carries no value fields |
...
`ruff format --check .`, `pytest tests/unit -q`. Badge in README.
...
- [ ] `scripts/generate_corpus.py` with seed 20260917 implementing §6;
      uniqueness assertion; writes 4 files + `manifest.json`
...
**Acceptance:** 4 docs + manifest committed; 45 questions with expected
outcomes committed and hand-verified; regeneration is byte-identical.
...
- [ ] `ingest/loaders.py`, `ingest/chunking.py` per §8 with tests
...
`valid_to` present); ~26 chunks indexed; `index` idempotent; CI green.
...
- [ ] `rate-rag eval --tune-threshold` implemented; run it; threshold +
      `tuned_on` written to `guardrails.yaml`; tuning file committed
...
**Session budget:** 0–3 ≈ 6 sessions, 4–6 ≈ 4, 7–8 ≈ 2, 9–10 ≈ 2 →
~14 two-hour sessions. At one session per weekday that is about three
weeks; target finish **second week of October 2026**.
...
4. Chroma vs. Pinecone parity row + latency.
...
6. Design notes: why no date filtering at retrieval (D-11), why baseline
   = gates off (D-13), why the corpus is synthetic, what LangChain pieces
   are used and which were deliberately not (D-04, D-05).
...
- [ ] `eval/results/` contains baseline + gated runs for both stores;
      ...
      `golden_accuracy ≥ 0.90`; parity ≤ 0.05
